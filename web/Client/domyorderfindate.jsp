...
<title>处理我的订单寻时间</title>
	...
	<body>
		...
		/*if(UserId==null)
		{
			out.print("<script>alert('请先登录！'); location.herf='login.jsp'</script>");
		}*/
		%>
	...
	String datetime=request.getParameter("datetime");// 2017/10/14
	String[] str = datetime.split("-");
	
	Connection conn1=DBHelper.getConn();
	String sql1="select MenuName,UserName,Tel,Address,OrderCount,OrderPrice,OrderSum,OrderDate,OrderPost  from Orders,Menu,Users where Orders.MenuId=Menu.MenuId and Orders.UserId=Users.UserId and Users.UserId=? and year(orderDate)=? and month(orderDate)=? and day(orderDate)=?" ;
	ResultSet rs=DBHelper.executeQuery(conn1, sql1,session.getAttribute("UserId"),str[0],str[1],str[2]);
	while (rs.next()){
%>
			<tr align="center">
